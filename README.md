...
./rag test        # engine suite: 64 tests + fixtures + byte-determinism, HERMETIC of the
                  # deployment's RAG_* env  (arms: python3 -m unittest discover -s tests → 9)
...
| `./rag test` | Engine unit + fixture e2e suite (64 tests; hermetic of the deployment's `RAG_*` env). |
...
- `./rag test`: 64 engine tests, covering unit detector controls and two fixture
  end-to-end runs (single-tier and tiered) with **byte-determinism asserts**; the test
  branch runs hermetic of the deployment's `RAG_*` exports (tests assume
